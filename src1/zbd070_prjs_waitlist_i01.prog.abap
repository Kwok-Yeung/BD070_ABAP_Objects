*----------------------------------------------------------------------*
***INCLUDE ZBD070_PRJS_WAITLIST_I01 .
*----------------------------------------------------------------------*
...
MODULE check_and_create_cust INPUT.
  ...
  IF sy-subrc <> 0.
    ...
    MESSAGE e916(zbd070) WITH scustom-id.
    ...
    WHEN 'CREATE'. "##########################################
      TRY.
        ...
        CATCH zcx_wait_list INTO r_exc.
          text = r_exc->get_text( ).
          MESSAGE s901(zbd070) WITH sdyn_conn-carrid sdyn_conn-connid
            sdyn_conn-fldate.
      ENDTRY.

    WHEN 'SHOW'.  "##########################################
      DATA: it_customer_list TYPE STANDARD TABLE OF zbd070_typd_cust.
      ...
      IF r_waitlist IS BOUND.
        ...
        IF r_customer_list IS NOT INITIAL.
          ...
          CALL FUNCTION 'ZBD070_DISPLAY_DATA'
            EXPORTING
...
*         The waitinglist for flight &1 &2 &3 has no entries !
        ...
        ENDIF.
      ELSE.
        MESSAGE s002(zbd070) WITH sdyn_conn-carrid sdyn_conn-connid
        sdyn_conn-fldate.
...
*         has been deleted.
        ...
        ENDTRY.
      ELSE.
        MESSAGE s902(zbd070) WITH sdyn_conn-carrid sdyn_conn-connid
        sdyn_conn-fldate.
...
*           Customer &1 was appended to waitinglist for flight &2 &3 &4
          CATCH zcx_wait_list INTO r_exc.  "wann könnte das passieren ???
            text = r_exc->get_text( ).
            MESSAGE text TYPE 'I'.
        ENDTRY.
...
*       Please select a valid customernumber !
      ...
      IF r_waitlist IS BOUND.
        IF NOT r_customer IS BOUND.
          MESSAGE s911(zbd070) WITH scustom-id sdyn_conn-carrid sdyn_conn-connid
               sdyn_conn-fldate..
        ELSE.
          TRY.
