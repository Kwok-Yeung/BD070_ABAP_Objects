...
CLASS lcl_customer DEFINITION.
  ...
  PUBLIC SECTION.
    ...
    METHODS get_attributes EXPORTING ex_cust TYPE zbd070_typd_cust.
...
CLASS lcl_waitlist DEFINITION.
  PUBLIC SECTION.
    ...
    METHODS get_first     RAISING   zcx_wait_list.
...
***    customer is already in wait_list for this flight !!!
      RAISE EXCEPTION TYPE zcx_wait_list
        EXPORTING
          textid   = zcx_wait_list=>cx_wait_list_customer_there
  ...
  METHOD get_first.      "--------------------------------
    ...
    IF sy-subrc <> 0.
      ...
      RAISE EXCEPTION TYPE zcx_wait_list
        EXPORTING
          textid = zcx_wait_list=>cx_wait_list_no_entry
          carrid = carrid
          connid = connid
          fldate = fldate.
    ELSE.
  ...
  METHOD delete.    "---------------------------------
    ...
    IF sy-subrc <> 0.
      ...
      RAISE EXCEPTION TYPE zcx_wait_list
        EXPORTING
          textid   = zcx_wait_list=>cx_wait_list_customer_notthere
          customer = im_customer->id
          carrid   = carrid
          connid   = connid
          fldate   = fldate.
    ...
    ELSE.
      RAISE EXCEPTION TYPE zcx_wait_list
        EXPORTING
          ...
          connid = im_connid
          fldate = im_fldate
          textid = zcx_wait_list=>cx_wait_list_exists.
    ENDIF.
